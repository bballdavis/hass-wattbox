...
class WattBoxHttpV24Compat:
    ...
    def get_device_info(self, refresh=False, include_outlet_power=True):
        # Return a dict similar to v2.4 get_device_info()
        return {
            'system_info': {
    ...
    def reset_all_outlets(self, delay=None):
        # Simulate master switch reset (turn off then on)
    ...
    def set_auto_reboot(self, enabled):
        # Not supported in HTTP API (read-only)
    ...
    # Expose any extra HTTP-only methods as needed
